...
interface IconHexagonNumber8FilledProps {
  className?: string;
  'data-slot'?: string;
  [key: string]: any;
}

const IconHexagonNumber8Filled: React.FC<IconHexagonNumber8FilledProps> = (props) => {
  // Handle size classes - if user provides size-* class, use it, otherwise default to size-4
  const userClassName = props.className || '';
  const hasSizeClass = /\bsize-\w+\b/.test(userClassName);
  const defaultSize = hasSizeClass ? '' : 'size-4';
  const finalClassName = `pelatform-icons pelatform-icon-hexagon-number-8-filled ${defaultSize} ${userClassName}`.trim();
...
export default IconHexagonNumber8Filled;