...
interface IconNumber82SmallProps {
  className?: string;
  'data-slot'?: string;
  [key: string]: any;
}

const IconNumber82Small: React.FC<IconNumber82SmallProps> = (props) => {
  // Handle size classes - if user provides size-* class, use it, otherwise default to size-4
  const userClassName = props.className || '';
  const hasSizeClass = /\bsize-\w+\b/.test(userClassName);
  const defaultSize = hasSizeClass ? '' : 'size-4';
  const finalClassName = `pelatform-icons pelatform-icon-number-82-small ${defaultSize} ${userClassName}`.trim();
...
export default IconNumber82Small;