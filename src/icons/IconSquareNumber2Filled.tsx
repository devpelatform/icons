...
interface IconSquareNumber2FilledProps {
  className?: string;
  'data-slot'?: string;
  [key: string]: any;
}

const IconSquareNumber2Filled: React.FC<IconSquareNumber2FilledProps> = (props) => {
  // Handle size classes - if user provides size-* class, use it, otherwise default to size-4
  const userClassName = props.className || '';
  const hasSizeClass = /\bsize-\w+\b/.test(userClassName);
  const defaultSize = hasSizeClass ? '' : 'size-4';
  const finalClassName = `pelatform-icons pelatform-icon-square-number-2-filled ${defaultSize} ${userClassName}`.trim();
...
export default IconSquareNumber2Filled;