...
interface IconHexagonNumber7FilledProps {
  className?: string;
  'data-slot'?: string;
  [key: string]: any;
}

const IconHexagonNumber7Filled: React.FC<IconHexagonNumber7FilledProps> = (props) => {
  // Handle size classes - if user provides size-* class, use it, otherwise default to size-4
  const userClassName = props.className || '';
  const hasSizeClass = /\bsize-\w+\b/.test(userClassName);
  const defaultSize = hasSizeClass ? '' : 'size-4';
  const finalClassName = `pelatform-icons pelatform-icon-hexagon-number-7-filled ${defaultSize} ${userClassName}`.trim();
...
export default IconHexagonNumber7Filled;