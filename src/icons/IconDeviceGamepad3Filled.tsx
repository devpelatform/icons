...
interface IconDeviceGamepad3FilledProps {
  className?: string;
  'data-slot'?: string;
  [key: string]: any;
}

const IconDeviceGamepad3Filled: React.FC<IconDeviceGamepad3FilledProps> = (props) => {
  // Handle size classes - if user provides size-* class, use it, otherwise default to size-4
  const userClassName = props.className || '';
  const hasSizeClass = /\bsize-\w+\b/.test(userClassName);
  const defaultSize = hasSizeClass ? '' : 'size-4';
  const finalClassName = `pelatform-icons pelatform-icon-device-gamepad-3-filled ${defaultSize} ${userClassName}`.trim();
...
export default IconDeviceGamepad3Filled;