...
interface IconBrandAdobeAfterEffectProps {
  className?: string;
  'data-slot'?: string;
  [key: string]: any;
}

const IconBrandAdobeAfterEffect: React.FC<IconBrandAdobeAfterEffectProps> = (props) => {
  // Handle size classes - if user provides size-* class, use it, otherwise default to size-4
  const userClassName = props.className || '';
  const hasSizeClass = /\bsize-\w+\b/.test(userClassName);
  const defaultSize = hasSizeClass ? '' : 'size-4';
  const finalClassName = `pelatform-icons pelatform-icon-brand-adobe-after-effect ${defaultSize} ${userClassName}`.trim();
...
export default IconBrandAdobeAfterEffect;