...
interface IconBatteryVertical3FilledProps {
  className?: string;
  'data-slot'?: string;
  [key: string]: any;
}

const IconBatteryVertical3Filled: React.FC<IconBatteryVertical3FilledProps> = (props) => {
  // Handle size classes - if user provides size-* class, use it, otherwise default to size-4
  const userClassName = props.className || '';
  const hasSizeClass = /\bsize-\w+\b/.test(userClassName);
  const defaultSize = hasSizeClass ? '' : 'size-4';
  const finalClassName = `pelatform-icons pelatform-icon-battery-vertical-3-filled ${defaultSize} ${userClassName}`.trim();
...
export default IconBatteryVertical3Filled;