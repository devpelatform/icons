...
interface IconView360NumberProps {
  className?: string;
  'data-slot'?: string;
  [key: string]: any;
}

const IconView360Number: React.FC<IconView360NumberProps> = (props) => {
  // Handle size classes - if user provides size-* class, use it, otherwise default to size-4
  const userClassName = props.className || '';
  const hasSizeClass = /\bsize-\w+\b/.test(userClassName);
  const defaultSize = hasSizeClass ? '' : 'size-4';
  const finalClassName = `pelatform-icons pelatform-icon-view-360-number ${defaultSize} ${userClassName}`.trim();
...
export default IconView360Number;