...
interface IconClockHour1FilledProps {
  className?: string;
  'data-slot'?: string;
  [key: string]: any;
}

const IconClockHour1Filled: React.FC<IconClockHour1FilledProps> = (props) => {
  // Handle size classes - if user provides size-* class, use it, otherwise default to size-4
  const userClassName = props.className || '';
  const hasSizeClass = /\bsize-\w+\b/.test(userClassName);
  const defaultSize = hasSizeClass ? '' : 'size-4';
  const finalClassName = `pelatform-icons pelatform-icon-clock-hour-1-filled ${defaultSize} ${userClassName}`.trim();
...
export default IconClockHour1Filled;