...
interface IconCircleNumber7FilledProps {
  className?: string;
  'data-slot'?: string;
  [key: string]: any;
}

const IconCircleNumber7Filled: React.FC<IconCircleNumber7FilledProps> = (props) => {
  // Handle size classes - if user provides size-* class, use it, otherwise default to size-4
  const userClassName = props.className || '';
  const hasSizeClass = /\bsize-\w+\b/.test(userClassName);
  const defaultSize = hasSizeClass ? '' : 'size-4';
  const finalClassName = `pelatform-icons pelatform-icon-circle-number-7-filled ${defaultSize} ${userClassName}`.trim();
...
export default IconCircleNumber7Filled;