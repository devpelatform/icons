...
interface IconZoomOutAreaFilledProps {
  className?: string;
  'data-slot'?: string;
  [key: string]: any;
}

const IconZoomOutAreaFilled: React.FC<IconZoomOutAreaFilledProps> = (props) => {
  // Handle size classes - if user provides size-* class, use it, otherwise default to size-4
  const userClassName = props.className || '';
  const hasSizeClass = /\bsize-\w+\b/.test(userClassName);
  const defaultSize = hasSizeClass ? '' : 'size-4';
  const finalClassName = `pelatform-icons pelatform-icon-zoom-out-area-filled ${defaultSize} ${userClassName}`.trim();
...
export default IconZoomOutAreaFilled;