...
interface IconCopyMinusFilledProps {
  className?: string;
  'data-slot'?: string;
  [key: string]: any;
}

const IconCopyMinusFilled: React.FC<IconCopyMinusFilledProps> = (props) => {
  // Handle size classes - if user provides size-* class, use it, otherwise default to size-4
  const userClassName = props.className || '';
  const hasSizeClass = /\bsize-\w+\b/.test(userClassName);
  const defaultSize = hasSizeClass ? '' : 'size-4';
  const finalClassName = `pelatform-icons pelatform-icon-copy-minus-filled ${defaultSize} ${userClassName}`.trim();
...
export default IconCopyMinusFilled;