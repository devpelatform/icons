...
interface IconBadge3dFilledProps {
  className?: string;
  'data-slot'?: string;
  [key: string]: any;
}

const IconBadge3dFilled: React.FC<IconBadge3dFilledProps> = (props) => {
  // Handle size classes - if user provides size-* class, use it, otherwise default to size-4
  const userClassName = props.className || '';
  const hasSizeClass = /\bsize-\w+\b/.test(userClassName);
  const defaultSize = hasSizeClass ? '' : 'size-4';
  const finalClassName = `pelatform-icons pelatform-icon-badge-3d-filled ${defaultSize} ${userClassName}`.trim();
...
export default IconBadge3dFilled;